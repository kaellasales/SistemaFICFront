...
import { Button } from '@/components/ui/Button'; // Corrigido para 'shared'
...
export function CourseFormPage() {
  ...
  const { formData, loading, saving, isEditing, handleChange, handleSubmit } = useCourseForm(id);
      ...
      <main className="max-w-4xl mx-auto px-6 py-8">
        <div className="flex items-center space-x-4 mb-8">
          ...
          <div>
            ...
            <p className="text-gray-600 mt-1">{isEditing ? 'Atualize as informações' : 'Preencha para agendar'}</p>
          </div>
        </div>
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="flex items-center justify-end space-x-4 pt-4 border-t">
            <Button type="button" variant="outline" onClick={() => navigate('/my-courses')} disabled={saving}>Cancelar</Button>
